...
var susyweb = new SusyWeb(); // Used only for its BigNumber library.
...
if (argv.d || argv.deterministic) {
  argv.s = "TestRPC is awesome!"; // Seed phrase; don't change to Susybraid, maintain original determinism
}
...
var server = Susybraid.server(options);

//console.log("Susybraid CLI v" + pkg.version);
console.log("SophonJS TestRPC v" + pkg.version);
...
server.listen(options.port, options.hostname, function(err, state) {
  ...
  if (options.fork) {
    ...
    console.log("Block:       " + susyweb.toBigNumber(state.blockchain.fork_block_number).toString(10));
